      ...
      for (let i = 0; i < retries; i++) {
        try {
          ...
          const unsubscribe = storeService.subscribe((stores) => {
            ...
            setProducts(prev => {
              // Filter out ANY product that comes from a physical store (by checking if its id contains an underscore following a store id pattern, or better, if its sellerId matches any store id)
              const storeIds = stores.map(s => s.id);
              const nonStoreProducts = prev.filter(p => !storeIds.includes(p.sellerId));
    ...
    <div className="min-h-screen flex flex-col bg-[#f3f4f6] pb-14">
      ...
      {activeStream && (
          <LiveStreamViewer 
            ...
            onPlaceBid={(product, amount) => {
              // Handle bid placement
              console.log(`Bid placed: ${product.title} - $${amount}`);
            }}
            onAddToCart={(product) => {
              // Handle add to cart
